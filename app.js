...
game.catchPokemon = function(pokemonObj) {
  // Add to party if there's space, otherwise add to collection
...
game.catchPokemon(snorlax);
game.catchPokemon(snorlax);
game.catchPokemon(snorlax); // this one goes to collection
...
pokemon.forEach(p => {
  // If this type doesn't exist yet, create an empty array
  if (!pokemonByType[p.type]) {
    pokemonByType[p.type] = [];
  }

  // Add this pokemon to its type array
